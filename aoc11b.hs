...
-- an item is defined as a list of its remainders when divided by the prime numbers
modlist :: Int -> Item
modlist n = [n `mod` y | y <- primes]

parseItemList :: String -> [Item]
parseItemList s = map (modlist . parseInt) (splitOn ',' (splitOn ':' s !! 1))
...
createOp :: [String] -> Op
createOp ["old",f,"old"] =  \m -> [parseFunc f x x `mod` y | (x,y) <- zip m primes]
createOp ["old",f,n] = \m -> [parseFunc f x (parseInt n) `mod` y | (x,y) <- zip m primes]
...
parseTest s i = fromJust (lookup n [x | x <- zip primes i ]) == 0
    where n = (parseInt . last . splitOn ' ') s
...
solution2 :: Input -> Int 
solution2 = product . take 2 . reverse . sort . map getCounter . monkeyBusiness 10000 . parseInput
